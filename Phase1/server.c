...
int handle_command(int sockfd) {
    ...
    if (userList[sockfd].state == UNVERIFIED) {
        ...
        // handle "get file"
            ...
            if (!put_file(filepath, &userList[sockfd])) {
                bzero(userList[sockfd].buf, MAX_BUFFER_SIZE);
                sprintf(userList[sockfd].buf, "The %s doesn’t exist\n", filename);
            
            } else {
                bzero(userList[sockfd].buf, MAX_BUFFER_SIZE);
                sprintf(userList[sockfd].buf, "get %s successfully\n", filename);
            }
            
            // send resulting message
            send(userList[sockfd].conn_fd, userList[sockfd].buf, strlen(userList[sockfd].buf), 0);
